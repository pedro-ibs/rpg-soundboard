...
function buildCardBody( sound ){
	const template = `<h6><strong>${sound.title}</strong></h6>
	...
	<div class="loop-toggle mb-2 d-flex justify-content-between align-items-center">
		<div>
			<input class="form-check-input loop-checkbox" type="checkbox" ${sound.loop ? 'checked' : ''}  id="id-sound-event-loop-${sound.id}" >
			<label class="form-check-label"> <i class="bi bi-repeat"></i></label>
		</div>

		<div>
			<input class="form-check-input loop-checkbox" type="checkbox" ${sound.playlist ? 'checked' : ''}  id="id-sound-event-playlist-${sound.id}" >
			<label class="form-check-label"><i class="bi bi-music-note-list"></i></label>
		</div>
...
function buildCard( sound, html_body, appState ){
	...
	const audioState = getAudioState(appState, sound.id);
	...
	if ( sound.playlist ) {
		stateClass += ' playlist-active';
		
		if (audioState === 'playing') {
			stateClass += ' playlist-current';
		}
	}
